...
def read_sh(fold_path, file_flag, rankflag, institute_flag):
    ...
    Args:
        fold_path (str or list): 文件夹路径,或文件夹路径列表,当类型为列表是,GRACE路径放在前,GRACE-FO路径放在后
        file_flag (str): 文件类型(GSM,GAA,GAC,GAD等)
        rankflag ([type]): 文件阶数标志(A->60,B->90,C->180)
        institute_flag (str): 文件来源(CSR,JPL,GFZ)

    Returns:
        Lmax, GC, GS, SC, file_num
    """
...
def read_GIA_sh(file_path,flag):
    ...
    Args:
        ...
        flag (str): GIA类型,Altimeter or GRACE
    """
    Lmax = 256
    linenum = sumN(Lmax+1)
    l = np.zeros(linenum)
